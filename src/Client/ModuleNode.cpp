...
void ModuleNode::setup()
{
    ...
    // yeezy: minimalistische Checkbox-Sprites
    auto cbOff = CCSprite::createWithSpriteFrameName("yz_checkbox_empty.png"_spr);
    auto cbOn = CCSprite::createWithSpriteFrameName("yz_checkbox_checked.png"_spr);
    if (cbOff && cbOn)
    {
        btn = CCMenuItemToggler::create(cbOff, cbOn, this, nullptr);
        btn->setScale(0.3f); // 96px -> ~29px
    }
    else
    {
...
{
    auto col = ccc3(140, 140, 137); // yeezy: graue Modulnamen (#8C8C89)
