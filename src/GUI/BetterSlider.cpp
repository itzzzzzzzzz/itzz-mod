...
void BetterSlider::ccTouchMoved(CCTouch *pTouch, CCEvent *pEvent)
{
    ...
    // itzz: slider-limit-Modul entfernt -> null-sicher (sonst Crash beim Slider-Bewegen)
    auto sliderLimitMod = Module::getByID("slider-limit");
    if (!(bypassAllowed && sliderLimitMod && sliderLimitMod->getRealEnabled()))
