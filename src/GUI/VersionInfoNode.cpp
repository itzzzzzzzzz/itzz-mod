...
void VersionInfoNode::updateLabel()
{
    ...
    switch (type)
    {
        ...
        case VersionInfoType::QOLMod:
            ...
            {
                label->setString(fmt::format("{}  |  itzz {}", LocalisationManager::get()->getLocalisedString("ui/update-available"), Mod::get()->getVersion().toVString()).c_str());
            }
            else
            {
                label->setString(fmt::format("itzz {}", Mod::get()->getVersion().toVString()).c_str());
            }
